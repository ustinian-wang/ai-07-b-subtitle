...
- kind: "read" | "query" | "mutate" | "cancel"
  - read: 纯分析/解释/讨论方案，不改库、不需拉列表
  - query: 列出/搜索/统计/读取笔记正文（含「读取内容」「看看未分类有哪些」）
...
- mutate_goal: "classify_by_city" | "move_records" | "generic" | null
  ...
  - generic: 其它写库操作或复合变更
- auto_execute: bool — 用户是否明确要求立刻执行（如「执行」「搬过去」「直接移动」「真正执行」）
  - 仅要方案/讨论/如果… → false
  - 明确执行/确认落库/直接移动 → true

注意：
- 不要输出工具名或工具参数
- 用户要求「读取笔记内容/正文」→ query，不是 read
- 用户说「按刚才分析直接移动」→ mutate + auto_execute=true
...
def route_by_rules(
    ...
    *,
    ...
    if re.search(r"读取|看看.*内容|抓取.*正文|include_text", text):
...
async def route(
    ...
    *,
    ...
    if t in ("ok", "okay", "好的", "好"):
        for m in reversed(history):
            if m.get("role") == "assistant":
                if re.search(r"读取|抓取|include_text|执行一次读取|去.*未分类", m.get("content") or ""):
                    return RouteDecision(kind="query")
                break
